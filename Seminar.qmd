...
Boolean possibility theory is based on propositional logic. The Pinciple of Bivalence states that every proposition is either true (1) or false (0). The current knowledge of an agent is represented by a \textit{belief base} $K$. If a proposition $p$ follows from $K$



Let $N(p) \in \{0, \, 1 \}$ denote the (Boolean) degree of  belief of $p$ (based on $K$). An agents state of belief is then represented by the pair $(N(p), \, N(\lnot p))$.
