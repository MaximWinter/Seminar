...
In the second case e.g. for $N(A \text{ is guilty}) = 1$ it directly follows through $\Pi(A) \text{ is not guilty}) = 1 -  N(A \text{ is guilty}) = 0$ and $A \text{ is guilty} \Rightleftarrow B \text{ is not guilty}$ that $B$ must be believed to be innocent.
\end{example}
